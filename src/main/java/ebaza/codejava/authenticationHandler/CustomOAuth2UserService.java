package ebaza.codejava.authenticationHandler;
...
public class CustomOAuth2UserService extends DefaultOAuth2UserService {
	...
	private OAuth2User processOAuth2User(OAuth2UserRequest oAuth2UserRequest, OAuth2User oAuth2User) {
		...
		if (userOptional.isPresent()) {
			...
			if (!user.getProvider()
					.equals(EProvider.valueOf(oAuth2UserRequest.getClientRegistration().getRegistrationId()))) {
	...
	private User registerNewUser(OAuth2UserRequest oAuth2UserRequest, OAuth2UserInfo oAuth2UserInfo) {
		...
		user.setCategory(ECategory.CLIENT);
		user.setUsername(oAuth2UserInfo.getEmail());
		user.setPassword(oAuth2UserInfo.getEmail());
	...
	private User updateExistingUser(User existingUser, OAuth2UserInfo oAuth2UserInfo) {
		...
		return userService.create(existingUser);
	}
